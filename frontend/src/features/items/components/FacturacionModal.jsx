...
const FacturacionModal = ({ isModalOpen, closeModal, cart, onSuccess }) => {
    ...
    return (
        <PopUp isModalOpen={isModalOpen} closeModal={closeModal}>
            <div className={`transition-all duration-500 ease-in-out ${isSplitViewOpen ? 'w-[1000px]' : 'w-[700px]'}`}>
                ...
                <div className="flex overflow-hidden">
                    ...
                    <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isSplitViewOpen ? 'w-[300px] p-4' : 'w-0'}`}>
                        <div className="bg-gray-800 text-white rounded-lg h-full p-4">
                            <h3 className="text-xl font-bold mb-4">Dividir la Cuenta</h3>
                            {cart.items_read.map( item =>(
                                <button key={item.id} className="w-full bg-gray-700 hover:bg-gray-600 rounded-lg p-3 mb-2 text-left font-medium cursor-pointer text-white">
